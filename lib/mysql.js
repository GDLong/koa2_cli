...
* 评论
 */
let replyList = function(value){
    let {articleId} = value;
    let _sql = `SELECT * FROM reply WHERE articleid = ${articleId} ORDER BY createdAt desc`
    return query(_sql)
}
let insertReply = function(value){
    let {articleId,commentId,content,userId} = value;
    if(!commentId) commentId = 0
    const date = (new Date()).toLocaleString();
    let _sql = `insert into reply(articleId,commentId,content,userId,createdAt,updatedAt)
                values(${articleId},${commentId},'${content}',${userId},'${date}','${date}')`
    return query(_sql)
}
module.exports = {
    registerUser,
